...
class Database:
        ...
        with self.cursor() as cur:
            execute_values(
                ...
                INSERT INTO broker_trades 
                    (broker_code, symbol, table_type, trade_date, netval, bval, sval, bavg, savg, crawl_timestamp)
                VALUES %s
                ON CONFLICT (broker_code, symbol, table_type, trade_date)
                DO UPDATE SET
    ...
    def _validate_trade(self, trade: dict) -> bool:
        ...
        required_fields = ["broker_code", "symbol", "table_type", "crawl_date"]
        ...
        # Validate table_type
        table_type = trade.get("table_type", "")
        if table_type not in ("buy", "sell"):
            logger.debug(f"Invalid table_type: {table_type}")
            return False
